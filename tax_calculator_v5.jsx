...
const TAX_DATA = {
  ...
  2026: {
    std_ded: 16100, medicare: 0.0145, fica: 0.062, fica_base: 180000,
    trs_emp: 0.0825, trs_utsw: 0.0825, orp_emp: 0.0665, orp_utsw: 0.085,
...
export default function App() {
  ...
  const [plan,setPlan]=useState("TRS");
  const [hasSec,setHasSec]=useState(false);
  const [ppFreq,setPpFreq]=useState("Monthly (12)");
  const mk=(iv,ifr)=>{const[v,sv]=useState(iv);const[f,sf]=useState(ifr);return[v,sv,f,sf];};
  const[pGrossV,setPGrossV,pGrossF,setPGrossF]=mk(100000,"Annually");
  const[pLonV,setPLonV,pLonF,setPLonF]=mk(0,"Annually");
  const[addWithV,setAddWithV,addWithF,setAddWithF]=mk(0,"Monthly");
  const[sGrossV,setSGrossV,sGrossF,setSGrossF]=mk(0,"Annually");
  const[wOtherV,setWOtherV,wOtherF,setWOtherF]=mk(0,"Monthly");
  const[t401V,setT401V,t401F,setT401F]=mk(0,"Monthly");
  const[t457V,setT457V,t457F,setT457F]=mk(0,"Monthly");
  const[tIraV,setTIraV,tIraF,setTIraF]=mk(0,"Annually");
  const[medV,setMedV,medF,setMedF]=mk(0,"Monthly");
  const[denV,setDenV,denF,setDenF]=mk(0,"Monthly");
  const[visV,setVisV,visF,setVisF]=mk(0,"Monthly");
  const[parkV,setParkV,parkF,setParkF]=mk(0,"Monthly");
  const[fsaV,setFsaV,fsaF,setFsaF]=mk(0,"Annually");
  const[ptOV,setPtOV,ptOF,setPtOF]=mk(0,"Monthly");
  const[r401V,setR401V,r401F,setR401F]=mk(0,"Monthly");
  const[r457V,setR457V,r457F,setR457F]=mk(0,"Monthly");
  const[rIraV,setRIraV,rIraF,setRIraF]=mk(0,"Annually");
  const[invV,setInvV,invF,setInvF]=mk(0,"Monthly");
  const[ptO2V,setPtO2V,ptO2F,setPtO2F]=mk(0,"Monthly");
  const[rentV,setRentV,rentF,setRentF]=mk(0,"Monthly");
  const[grocV,setGrocV,grocF,setGrocF]=mk(0,"Monthly");
  const[carIV,setCarIV,carIF,setCarIF]=mk(0,"Monthly");
  const[rIV,setRIV,rIF,setRIF]=mk(0,"Monthly");
  const[inetV,setInetV,inetF,setInetF]=mk(0,"Monthly");
  const[elecV,setElecV,elecF,setElecF]=mk(0,"Monthly");
  const[utilV,setUtilV,utilF,setUtilF]=mk(0,"Monthly");
  const[livOV,setLivOV,livOF,setLivOF]=mk(0,"Monthly");

  // Net worth
  const[nwCheck,setNwCheck]=useState(0);const[nwHys,setNwHys]=useState(0);
  const[nw401,setNw401]=useState(0);const[nw457,setNw457]=useState(0);
  const[nwIra,setNwIra]=useState(0);const[nwBrok,setNwBrok]=useState(0);
  const[nwHome,setNwHome]=useState(0);const[nwCar,setNwCar]=useState(0);
  const[nwOA,setNwOA]=useState(0);
  const[nwMort,setNwMort]=useState(0);const[nwCL,setNwCL]=useState(0);
  const[nwStu,setNwStu]=useState(0);const[nwCC,setNwCC]=useState(0);
  const[nwOD,setNwOD]=useState(0);
  const[pYears,setPYears]=useState(25);const[pRate,setPRate]=useState(7);const[initBal,setInitBal]=useState(0);

  // Pension
  const[trsSals,setTrsSals]=useState([0,0,0,0,0]);
  const[trsYears,setTrsYears]=useState(0);const[trsMult,setTrsMult]=useState(2.3);

  // Retirement predictor
  const[retYear,setRetYear]=useState(2055);
  const[retAnnRate,setRetAnnRate]=useState(9);
  const[retEffTax,setRetEffTax]=useState(18);
  const[retLtcg,setRetLtcg]=useState(15);
  const[retCompRate,setRetCompRate]=useState(0);
  const[prevTradBal,setPrevTradBal]=useState(0);
  const[prevRothBal,setPrevRothBal]=useState(0);
  const[customSplitPct,setCustomSplitPct]=useState(50);
  const[nwYears,setNwYears]=useState(25);
